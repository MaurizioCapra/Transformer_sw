...
train_stream = trax.data.tf_inputs.TFDS('wmt14_translate', keys=('en', 'de'), train=True)()
eval_stream = trax.data.tf_inputs.TFDS('wmt14_translate', keys=('en', 'de'), train=False)()
...
#    trax.data.tf_inputs.Tokenize(vocab_file='ende_32k.subword'),
#    trax.data.Shuffle(),
#    trax.data.FilterByLength(max_length=input_sequence_length),
#    # trax.data.PadToLength(),
#    # trax.data.Batch(BATCH_SIZE),
#    trax.data.BucketByLength(boundaries=[128],
#                             batch_sizes=[BATCH_SIZE, BATCH_SIZE],
#                             strict_pad_on_len=True),
#    # trax.data.AddLossWeights() keys=[0] length_keys=[0],
#  )

EOS =  list(trax.data.tf_inputs.tokenize(['EOS'], vocab_file="ende_32k.subword"))[0]
