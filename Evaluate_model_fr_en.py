...
val_examples_tok = np.load("wmt14-fr-en_tok.npy", allow_pickle=True)
...
end = list(trax.data.tf_inputs.tokenize(['stop'], vocab_file="endefr_32k.subword"))[0]
...
def evaluate(sentence, max_length=input_sequence_length):
    ...
    if (output[0][-1])!=end:
        ...
        text = trax.data.detokenize(output[0][1:], vocab_file="endefr_32k.subword")
    else:
        text = trax.data.detokenize(output[0][1:-1], vocab_file="endefr_32k.subword")  # shape: ()
...
perform_BLEU = "sacrebleu -t wmt14 -l fr-en"
